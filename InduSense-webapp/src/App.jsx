...
import { ref, onValue, query, orderByChild, startAt, set, update } from "firebase/database";
...
const BASE_THRESHOLD = 27;
const PUMP_ON_LEVEL = 5;
const TANK_HEIGHT = 6;
...
export default function App() {
  ...
  // --- Firebase: sensors + commands listeners ---
  useEffect(() => {
    const un1 = onValue(ref(db, SENSORS_PATH), snap => {
      const d = snap.val();
    ...
    const un2 = onValue(ref(db, COMMANDS_PATH), snap => {
      const d = snap.val();
  ...
  useEffect(() => {
    ...
    const unsub = onValue(q, snap => {
      const d = snap.val();
      if (!d) { setHistory([]); setLoadingHistory(false); return; }
  ...
  const analytics = useMemo(() => {
    if (!history || history.length === 0)
      return { avgTemp: null, gasCount: 0, pumpOnMinutes: 0, fanOnMinutes: 0, alertCount: 0 };
    ...
    const lastInterval = Math.max(0, Date.now() - last.ts);
    if (last.pump === 1) pumpMs += lastInterval;
    if (last.fan === 1) fanMs += lastInterval;

    return {
      avgTemp: avgTemp === null ? null : +avgTemp.toFixed(2),
      gasCount,
      pumpOnMinutes: +(pumpMs / 60000).toFixed(2),
      fanOnMinutes: +(fanMs / 60000).toFixed(2),
      alertCount: gasCount
    };
  }, [history]);
  ...
  const dynamicThreshold = useMemo(() => {
    if (!history || history.length === 0) return BASE_THRESHOLD;
    const temps = history.filter(h => h.temp !== null).map(h => Number(h.temp));
    if (temps.length === 0) return BASE_THRESHOLD;
    const avg = temps.reduce((a, b) => a + b, 0) / temps.length;
    const seasonalOffset = analytics.avgTemp !== null && analytics.avgTemp > 30 ? 2 : 1.5;
    return +Math.max(BASE_THRESHOLD, avg + seasonalOffset).toFixed(2);
  }, [history, analytics.avgTemp]);

  // --- Write dynamic threshold to Firebase for ESP32 to read ---
  useEffect(() => {
    const valueToWrite = (!loadingHistory && history.length > 0)
      ? dynamicThreshold
      : BASE_THRESHOLD;

    set(ref(db, "ml/thresholds/fan"), valueToWrite)
      .catch(err => console.error("❌ Threshold write failed:", err.code));
  }, [dynamicThreshold, loadingHistory]);

  // --- Effective actuator states ---
  const effective = useMemo(() => ({
    fan: commands.fan !== null ? commands.fan : (sensors.temp !== null && sensors.temp > dynamicThreshold),
    pump: commands.pump !== null ? commands.pump : (sensors.distance !== null && sensors.distance >= PUMP_ON_LEVEL),
    buzzer: commands.buzzer !== null ? commands.buzzer : (sensors.mq135 || sensors.mq2)
  }), [commands, sensors, dynamicThreshold]);

  const tempSeries = useMemo(() =>
    history.filter(h => h.temp !== null).map(h => ({ ts: h.ts, temp: Number(h.temp) })),
    [history]);
  ...
  const doSendCommand = (key, value) => {
    const obj = { fan: commands.fan, pump: commands.pump, buzzer: commands.buzzer, ts: Date.now(), device: DEVICE_ID };
    obj[key] = value;
    set(ref(db, COMMANDS_PATH), obj).catch(err => console.error("Command write error:", err));
  };
  ...
  const fanShouldBeOn = sensors.temp !== null && sensors.temp > dynamicThreshold;
  ...
  const fluidPercentage = useMemo(() => {
    ...
    return Math.round(((TANK_HEIGHT - clamped) / (TANK_HEIGHT - 1)) * 100);
  }, [sensors.distance]);
      ...
      <header className="header">
        <div className="header-content">
          ...
          <div className="header-controls">
            <button className={`refresh-btn ${isRefreshing ? 'refreshing' : ''}`} onClick={handleRefresh} disabled={isRefreshing}>
        ...
        <div className="system-indicators">
          <div className={`indicator ${gasDetected ? 'critical' : 'safe'}`}>
            <div className="indicator-light"></div><span>Gas Detection</span>
          </div>
          <div className={`indicator ${fanWarning ? 'warning' : 'safe'}`}>
            <div className="indicator-light"></div><span>Fan Status</span>
          </div>
          <div className={`indicator ${pumpWarning ? 'warning' : 'safe'}`}>
            <div className="indicator-light"></div><span>Pump Status</span>
          </div>
          <div className={`indicator ${isOnline ? 'safe' : 'critical'}`}>
            <div className="indicator-light"></div><span>System Connection</span>
          </div>
        </div>
      </header>
      ...
      <main className="grid">
        <section className="left-main">
            ...
            <div className="sensor-grid">
              <div className="sensor-card">
                ...
                <div className="sensor-data">
                  ...
                  <div className="sensor-info">Auto fan &gt; {dynamicThreshold}°C</div>
                </div>
              </div>
              <div className="sensor-card">
              ...
              <div className="sensor-card">
                ...
                <div className="sensor-data">
                  ...
                  <div className="sensor-value">{sensors.distance === null ? "—" : `${fluidPercentage}%`}</div>
                  <div className="sensor-info">
                    {sensors.distance === null ? "No data" : `Level: ${sensors.distance} cm | Auto pump ≥ ${PUMP_ON_LEVEL}cm`}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="card chart-card">
            ...
            {loadingHistory ? (
              <div className="loading-state"><div className="spinner"></div><span>Loading history...</span></div>
            ) : (
          ...
          <section className="card controls-card">
            ...
            <div className="control-grid">
              <div className={`control-item ${commands.fan !== null ? 'manual-active' : ''}`}>
                <div className="control-header">
                  <div className="control-info">
                    ...
                    <p className="control-status">
                      {commands.fan === null ? `Auto (>${dynamicThreshold}°C)` : (commands.fan ? "Manual ON" : "Manual OFF")}
                    </p>
                  </div>
                ...
                <ToggleSwitch checked={commands.fan ?? false} onChange={v => doSendCommand("fan", v)} />
              </div>
              <div className={`control-item ${commands.pump !== null ? 'manual-active' : ''}`}>
                ...
                <ToggleSwitch checked={commands.pump ?? false} onChange={v => doSendCommand("pump", v)} />
              </div>
              <div className={`control-item ${commands.buzzer !== null ? 'manual-active' : ''} ${gasDetected ? 'alert-active' : ''}`}>
                ...
                <ToggleSwitch checked={commands.buzzer ?? false} onChange={v => doSendCommand("buzzer", v)} />
              </div>
            </div>
          ...
          <section className="card system-info-card">
            ...
            <div className="info-grid">
              ...
              <div className="info-item">
                ...
                <div className="info-value">{isOnline ? formatTimeAgo(lastUpdate) : "Offline"}</div>
              </div>
              <div className="info-item">
            ...
            <div className="thresholds-section">
              ...
              <div className="threshold-list">
                <div className="threshold-item">
                  ...
                  <span className="threshold-value">&gt; {dynamicThreshold}°C</span>
                </div>
                <div className="threshold-item">
